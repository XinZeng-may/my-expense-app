...
with tab2:
    st.subheader("💵 现金流统计")
    st.caption("逻辑：现金/借记卡当天流出；信用卡消费先进入待还，只有手动记录还款后才算已实际流出；工资按规则自动生成。")
    ...
    if not cash_source_df.empty:
        ...
        cash_source_df = pd.DataFrame(columns=list(df.columns) + ["adjusted_amount"])

    # =========================
    # 2. 信用卡消费 + cashback 规则
    # =========================
    credit_charge_df = cash_source_df[
        (cash_source_df["payment_method"] == "信用卡")
        & (cash_source_df["card_name"].fillna("") != "")
    ].copy()

    if not credit_charge_df.empty:
        credit_charge_df = apply_cashback_rules(
            credit_charge_df,
            cards_df,
            cashback_rules_df,
        )
    ...
    # 3. 信用卡还款记录
    # =========================
    payments_df = credit_card_payments_df.copy() if not credit_card_payments_df.empty else pd.DataFrame()
    ...
    # =========================
    # 4. 当前待还汇总（先算出来，后面还款表单要用）
    # =========================
    if credit_charge_df.empty:
        current_outstanding_summary = pd.DataFrame(
            columns=["信用卡", "总消费金额", "总预计cashback", "属于谁", "每月还款日", "已还金额", "当前待还金额", "待还cashback", "下一次还款日期"]
        )
    else:
        charge_summary = (
            credit_charge_df.groupby("card_name", as_index=False)
            .agg(
        ...
        if not cards_df.empty:
            ...
            card_meta_df = pd.DataFrame(columns=["信用卡", "属于谁", "每月还款日"])

        current_outstanding_summary = charge_summary.merge(card_meta_df, on="信用卡", how="left")
        current_outstanding_summary = current_outstanding_summary.merge(payment_summary, on="信用卡", how="left")
        current_outstanding_summary["已还金额"] = current_outstanding_summary["已还金额"].fillna(0.0)

        current_outstanding_summary["当前待还金额"] = (
            current_outstanding_summary["总消费金额"] - current_outstanding_summary["已还金额"]
        ).clip(lower=0)

        # 剩余待还对应的 cashback，按比例折算
        current_outstanding_summary["待还cashback"] = current_outstanding_summary.apply(
            lambda row: (
        ...
        current_outstanding_summary["下一次还款日期"] = current_outstanding_summary["每月还款日"].apply(get_next_due_date)

        current_outstanding_summary = current_outstanding_summary[current_outstanding_summary["当前待还金额"] > 0].copy()
        current_outstanding_summary = current_outstanding_summary.sort_values("当前待还金额", ascending=False)

    outstanding_card_names = current_outstanding_summary["信用卡"].tolist() if not current_outstanding_summary.empty else []
    outstanding_map = {}
    if not current_outstanding_summary.empty:
        outstanding_map = dict(
            zip(current_outstanding_summary["信用卡"], current_outstanding_summary["当前待还金额"])
        )
    ...
    if not outstanding_card_names:
        st.info("当前没有待还的信用卡。")
    else:
        selected_repay_card_preview = st.selectbox(
            "选择要还款的信用卡（查看当前待还）",
        ...
        with st.form("add_credit_card_payment_form", clear_on_submit=True):
            ...
            with rp2:
                repayment_card_name = st.selectbox(
                    "信用卡",
                    outstanding_card_names,
                    index=outstanding_card_names.index(selected_repay_card_preview),
                    key="repayment_card"
                )
            with rp3:
                repay_full = st.checkbox("本次还全额", value=True, key="repay_full_checkbox")

                default_repay_amount = float(outstanding_map.get(selected_repay_card_preview, 0.0))
                repayment_amount = st.number_input(
                    "还款金额",
                    min_value=0.0,
                    value=default_repay_amount,
                    step=10.0,
                    format="%.2f",
            ...
            with rp5:
                repayment_note = st.text_input(
                    "备注",
                    placeholder="例如：4月账单全额还款 / 提前部分还款",
                    key="repayment_note"
                )
        ...
        if submitted_repayment:
            # 如果勾选全额，就自动覆盖金额
            actual_repayment_amount = float(outstanding_map.get(repayment_card_name, 0.0)) if repay_full else float(repayment_amount)
    ...
    pending_card_amount = current_outstanding_summary["当前待还金额"].sum() if not current_outstanding_summary.empty else 0.0
    ...
    if not payment_events_in_range.empty:
        ...
        pay_schedule["日期"] = payment_events_in_range["payment_date"].dt.date.astype(str)
    ...
    if current_outstanding_summary.empty:
        st.info("当前没有待还信用卡。")
    else:
        show_outstanding_df = current_outstanding_summary[
            ["信用卡", "当前待还金额", "属于谁", "每月还款日", "下一次还款日期", "待还cashback", "已还金额"]
        ].copy()

        st.dataframe(show_outstanding_df, use_container_width=True, hide_index=True)
with tab3:
    st.subheader("💳 信用卡管理")
